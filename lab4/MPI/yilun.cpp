...
// }
                                             ...
                                             int worker_rank)
    {
        ...
        std::vector<double> diag_super(2 * q - 1, 0.0);
        ...
        int rot_meta[4] = {0, 0, 0, 0};

        t0 = MPI_Wtime();

        MPI_Recv(rot_meta, 4, MPI_INT, worker_rank,
        ...
        g_mpi_worker_compute_time += rot_double[2 * rot_count];
        ++g_mpi_svd_tasks;
...
bool gkh_svd_from_bidiagonal(Matrix &U, Matrix &B, Matrix &V, int max_iter, double tol)
{
    ...
    for (int iter = 0; iter < max_iter; ++iter)
    {
        ...
        // 从右到左处理每个非平凡块。
// MPI 版本：优先把 block 的 Givens 旋转计算交给 worker，
// master 收回旋转序列后在全局 B/U/V 上重放。
// 若 MPI 不可用，则回退到原始串行 one_block_step。
int next_worker = 1;
